...
@section('content')
    <div class="mx-auto max-w-2xl">
        <div class="mb-8">
            <h1 class="text-2xl font-bold tracking-tight text-zinc-950">Edit Location</h1>
        </div>
        ...
        <div class="rounded-lg border border-zinc-200 bg-white shadow-sm">
            <form action="{{ route('locations.update', $location->id) }}" method="POST" class="p-6 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="type"
                        class="flex h-10 w-full rounded-md border border-zinc-200 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">
                        <option value="Pit" {{ $location->type == 'Pit' ? 'selected' : '' }}>Pit</option>
                        <option value="Stockpile" {{ $location->type == 'Stockpile' ? 'selected' : '' }}>Stockpile</option>
                        <option value="Crusher" {{ $location->type == 'Crusher' ? 'selected' : '' }}>Crusher</option>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="3"
                        class="flex w-full rounded-md border border-zinc-200 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950">{{ $location->description }}</textarea>
                </div>
